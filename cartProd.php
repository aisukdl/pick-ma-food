...
<body>
 <a href="prodByProd.php" class="button">Back</a>
<?php 
session_start();
						...
						foreach($_SESSION["shopping_cart"] as $keys => $values)
							...
							if (!$res) 
							...
							else
								{
									...
									echo "<td>". $tot ." THB</td>";?>
									<td> <a href='cartProd.php?action=delete&id=<?php echo $pID; ?>'> <span class='text-danger'> Remove </span></a></td>
									<?php
									$total = $total + $tot;
									echo "</tr>";
								}
	...
	if($_GET["action"] == "delete")
	{
		foreach($_SESSION["shopping_cart"] as $keys => $values)
		{
			if($values["item_id"] == $_GET["id"])
			{
				...
				echo '<script>window.location="cartProd.php"</script>';
			}
		}
	}
}
